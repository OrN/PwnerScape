package com.pwns.server.model.entity.player;
...
import com.pwns.server.GameStateUpdater;
import com.pwns.server.content.clan.ClanPlayer;
import com.pwns.server.event.DelayedEvent;
import com.pwns.server.event.custom.BatchEvent;
import com.pwns.server.login.LoginRequest;
import com.pwns.server.model.action.WalkToAction;
...
import com.pwns.server.plugins.QuestInterface;
import com.pwns.server.sql.query.logs.LiveFeedLog;
...
import com.pwns.server.content.clan.Clan;
import com.pwns.server.content.clan.ClanInvite;
import com.pwns.server.event.rsc.impl.FireCannonEvent;
import com.pwns.server.event.rsc.impl.PoisonEvent;
import com.pwns.server.event.rsc.impl.PrayerDrainEvent;
...
import com.pwns.server.model.container.Bank;
import com.pwns.server.model.container.Inventory;
...
import com.pwns.server.net.rsc.PacketHandlerLookup;
...
public final class Player extends Mob {
	...
	public final int SKULL_TIMER = 1200000;
	...
	public boolean checkAttack(Mob mob, boolean missile) {
		if (mob.isPlayer()) {
			...
			if (!victim.isPVP(this) || victim.getAttribute("no-aggro", false)) {
	...
	public boolean isInsideSafeZone() {
		...
		if (floor <= 2) {
			// Tutorial Island
			if (x <= 241 && y <= 770 && x >= 186 && y >= 719)
				return true;

			// Edgeville
			if (x <= 234 && y <= 475 && x >= 202 && y >= 431)
				return true;

			// Varrock
			if (x <= 157 && y <= 537 && x >= 77 && y >= 520)
				return true;
			if (x <= 185 && y <= 519 && x >= 86 && y >= 475)
				return true;
			if (x <= 150 && y <= 474 && x >= 89 && y >= 444)
				return true;

			// Lumbridge
			if (x <= 143 && y <= 678 && x >= 101 && y >= 635)
				return true;
			if (x <= 135 && y <= 634 && x >= 125 && y >= 621)
				return true;

			// Draynor
			if (x <= 232 && y <= 640 && x >= 209 && y >= 607)
				return true;
			if (x <= 208 && y <= 623 && x >= 194 && y >= 607)
				return true;

			// Falador
			if (x <= 342 && y <= 583 && x >= 246 && y >= 518)
				return true;

			// Al Kharid
			if (x <= 93 && y <= 722 && x >= 48 && y >= 659)
				return true;
			if (x <= 67 && y <= 734 && x >= 58 && y >= 722)
				return true;
			if (x <= 102 && y <= 711 && x >= 94 && y >= 670)
				return true;

			// Shilo Village
			if (x <= 432 && y <= 862 && x >= 383 && y >= 816)
				return true;

			// Yanille
			if (x <= 648 && y <= 769 && x >= 578 && y >= 742)
				return true;

			// East Ardougne
			if (x <= 623 && y <= 627 && x >= 533 && y >= 566)
				return true;

			// Fishing Guild (Bank)
			if (x <= 605 && y <= 509 && x >= 599 && y >= 500)
				return true;

			// Seer's Village
			if (x <= 527 && y <= 479 && x >= 488 && y >= 446)
				return true;

			// Catherby
			if (x <= 453 && y <= 509 && x >= 431 && y >= 478)
				return true;
			if (x <= 430 && y <= 494 && x >= 416 && y >= 478)
				return true;

			// Tree Gnome Stronghold
			if (x <= 751 && y <= 542 && x >= 672 && y >= 432)
				return true;
		}

		// Mage Arena Bank
		if (x <= 479 && yOrig <= 3407 && x >= 432 && yOrig >= 3360)
				return true;

		// Grand Tree
		if (x <= 431 && y <= 191 && x >= 384 && y >= 144)
			return true;

		return false;
	}

	public boolean isPVP(Player otherPlayer) {
		// Check other player
		if (otherPlayer != null) {
			if (clan != null) {
				// If other player is in our clan, make them non-pvp
				ClanPlayer clanPlayer = clan.getPlayer(otherPlayer.getUsername());
				if (clanPlayer != null)
					return false;
			}
		}

		// Always pvp flag if you have skull
		if (isSkulled())
			return true;
	...
	public boolean getPVPMode() {
		if (getCache().hasKey("pvp_mode")) {
			return getCache().getBoolean("pvp_mode");
		}
		return false;
	}

	public void setPVPMode(boolean value) {
		getCache().store("pvp_mode", value);
	}

	public void togglePVP() {
		if (!isInsideSafeZone()) {
			message("You need to be in a @yel@Safe@whi@ area to switch modes");
			return;
		}
		...
		if (getPVPMode())
			message("You are now set to @red@PVP@whi@ mode");
		else
			message("You are now set to @yel@PVE@whi@ mode");
	}

	public void setSkulledOn(Player player) {
		player.getSettings().addAttackedBy(this);
		if (player.isInsideSafeZone() || System.currentTimeMillis() - getSettings().lastAttackedBy(player) > SKULL_TIMER) {
			addSkull(SKULL_TIMER);
		}
	...
	public void setClan(Clan clan, boolean updateOthers) {
		Clan prevClan = this.clan;
		this.clan = clan;

		// TODO: PVP clan leaving flag update, make sure this doesn't crash clients
		if (updateOthers) {
			// Update local players on clan changes
			for (Player player : localPlayers) {
				if (player != this && ((prevClan != null && player.clan == prevClan) || player.clan == this.clan))
					player.getUpdateFlags().setAppearanceChanged(true);
			}
		}
