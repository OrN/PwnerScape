...
import com.pwns.server.event.rsc.impl.combat.CombatEvent;
...
public class PlayerDuelHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, Player player) throws Exception {
		...
		if (player.isPVP()) {
			player.message("You cannot duel while flagged for @red@PVP@whi@!");
			return;
		}
		...
		if (pID == packetFour) { // Sending duel request
			...
			if (affectedPlayer.isPVP()) {
				player.message("They cannot duel while flagged for @red@PVP@whi@!");
				return;
			}
