...
public class JungleForester implements TalkToNpcListener, TalkToNpcExecutiveListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener {
	...
	private void LegendsQuest_jungleForesterDialogue(Player p, Npc n, int cID) {
		if(n.getID() == JUNGLE_FORESTER) {
			if(cID == -1) {
				...
				case 11:
				case -1:
					Functions.npcTalk(p, n, "Hello friend, you're a long way from civilisation!");
					int opt = Functions.showMenu(p, n,
			...
			case JungleForesterNPC_LegendsQuest.WHAT_DO_YOU_DO_HERE:
				Functions.npcTalk(p, n, "I'm a forester, and I specialise in exotic woods. ",
			...
			case JungleForesterNPC_LegendsQuest.NATIVES_IN_THE_JUNGLE:
				Functions.npcTalk(p, n, "Well, I've heard some funny sounds...",
				...
				int opt = Functions.showMenu(p, n,
						...
						"Ok thanks");
				if(opt == 0) {
					Functions.npcTalk(p, n, "Well, I wish I could give it to you.",
							"However, I have grown fond of it.",
							"And it may help me incase I get lost in the jungle.");
					int opt3 = Functions.showMenu(p, n,
							"Will you trade something for it?",
							"Ok thanks");
					if(opt3 == 0) {
						Functions.npcTalk(p, n, "Well, if you have something interesting, let me have a look at it",
			...
			case JungleForesterNPC_LegendsQuest.KHARAZI_JUNGLE:
				Functions.npcTalk(p, n, "Well, I've not managed it yet, ",
				...
				int option = Functions.showMenu(p, n,
						...
						"Ok thanks");
				if(option == 0) {
					Functions.message(p, 1200, "The forester looks very interested..");
					Functions.npcTalk(p, n, "Oh, well, that sounds quite good actually...",
					...
					Functions.message(p, 300, "The forester looks very thoughtfull");
					Functions.npcTalk(p, n, "Hey, if you manage to complete it, be sure to let me take a look!",
							"Well, best of luck with it, I'm sure you're going to need it.");
					int opt2 = Functions.showMenu(p, n,
							"Do you have any other tips about the Kharazi jungle?",
							"Have you seen any natives in the jungle?",
							"Ok thanks");
					if(opt2 == 0) {
						Functions.npcTalk(p, n, "Not really, but I would say be careful, it's a dangerous place.",
								"And good luck.");
					} else if(opt2 == 1) {
						LegendsQuest_jungleForesterDialogue(p, n, JungleForesterNPC_LegendsQuest.NATIVES_IN_THE_JUNGLE);
					} else if(opt2 == 2) {
					...
					Functions.npcTalk(p, n, "No, of course not...",
			...
			case JungleForesterNPC_LegendsQuest.MAKE_A_COPY:
				Functions.npcTalk(p, n, "Many thanks friend.");
				Functions.message(p, 1200, "The Jungle Forester takes out some parchment and some charcoal.",
						"He studiously renders another copy of your map.");
				Functions.npcTalk(p, n, "Many thanks friend.");
				Functions.message(p, 1200, "He takes out a strange looking object and hands it to you.");
				Functions.npcTalk(p, n, "Here, I won't be needing this any longer, and it may help you.",
						"Whenever I've used it before, it attracted the attention of jungle natives.");
				Functions.addItem(p, 1177, 1);
				break;
			}
		}
	}
	...
	private void defaultJungleForesterDialogue(Player p, Npc n, int cID) {
		if(n.getID() == JUNGLE_FORESTER) {
			...
			case JungleForesterNPC.WHO_ARE_YOU:
				Functions.npcTalk(p, n, "I'm a jungle forester,",
						"Names mean little in this part of the world.");
				int sub_menu = Functions.showMenu(p, n,
						"What do you do here?",
						"How do I get into the Kharazi jungle?");
				if(sub_menu == 0) {
					defaultJungleForesterDialogue(p, n, JungleForesterNPC.WHAT_DO_YOU_DO_HERE);
			...
			case JungleForesterNPC.KHARAZI_JUNGLE:
				Functions.npcTalk(p, n, "Well, I've not managed it yet, ",
						...
						"Apparently he was on a mission to map the area.",
						"How foolish is that?");
				int sub_menu2 = Functions.showMenu(p, n,
						"So someone managed to get into the Kharazi Jungle?",
						"What do you do here?",
						"Ok thanks");
				if(sub_menu2 == 0) {
					Functions.npcTalk(p, n, "Yes, he said he was from some place...near the Barbarian outpost.",
							"Mentioned something about a legend ?",
							"It meant nothing to me though.");
					int sub_menu3 = Functions.showMenu(p, n,
							"How do I get into the jungle?",
							"What do you do here?",
							"Ok thanks");
					if(sub_menu3 == 0) {
	...
	public void onInvUseOnNpc(Player p, Npc n, Item item) {
		if(n.getID() == JUNGLE_FORESTER && item.getID() == 1233) { // the complete map.
			...
			if(Functions.hasItem(p, 1177)) { // if already have the bull roarer
				Functions.npcTalk(p, n, "It's a great map, thanks for letting me take a copy!",
						"It has helped me out a number of times now.");
				return;
			}
			Functions.npcTalk(p, n, "*Gasp*");
			p.message("The jungle forester looks speechless.");
			Functions.npcTalk(p, n, "This is very impressive!",
			...
			if(menu == 0) {
				...
				Functions.npcTalk(p, n, "Well, I can offer you this?");
				Functions.message(p, 1200, "The Jungle Forester takes out a strange looking object.",
				...
				Functions.npcTalk(p, n, "If you swing this above your head, it makes a strange sound.",
				...
				if(opt == 0) {
					...
					Functions.npcTalk(p, n, "Very well friend, I understand, I must be on my way as well.");
