...
import com.pwns.server.content.market.Market;
...
public class Bankers implements TalkToNpcExecutiveListener, TalkToNpcListener, NpcCommandListener, NpcCommandExecutiveListener {
	...
	public boolean blockTalkToNpc(final Player player, final Npc npc) {
		if(Functions.inArray(npc.getID(), BANKERS)) {
			return true;
		}
		return false;
	}
	...
	public void onTalkToNpc(Player player, final Npc npc) {
		Functions.npcTalk(player, npc, "Good day"+(npc.getID() == 617 ? " Bwana" : "")+", how may I help you?");
		int menu = Functions.showMenu(player, npc,
		...
		if (menu == 0) {
			...
			menu = Functions.showMenu(player, "Set a bank pin", "Change bank pin", "Delete bank pin");
			if (menu == 0) {
				if (!player.getCache().hasKey("bank_pin")) {
					String bankPin = Functions.getBankPinInput(player);
					if (bankPin == null) {
						return;
					}
				...
				if (player.getCache().hasKey("bank_pin")) {
					...
					String changeTo = Functions.getBankPinInput(player);
				...
				if (player.getCache().hasKey("bank_pin")) {
					...
					if(player.getIronMan() > 0 && player.getIronManRestriction() == 0) {
						Functions.message(player, npc, 1000, "Deleting your bankpin results in permanent iron man restriction",
								"Are you sure you want to do it?");
						
						int deleteMenu = Functions.showMenu(player, "I want to do it!",
								"No thanks.");
						if(deleteMenu == 0) {
			...
			Functions.npcTalk(player, npc, "This is a branch of the bank of Runescape", "We have branches in many towns");
			int branchMenu = Functions.showMenu(player, npc, "And what do you do?",
					"Didn't you used to be called the bank of Varrock");
			if (branchMenu == 0) {
				Functions.npcTalk(player, npc, "We will look after your items and money for you",
						"So leave your valuables with us if you want to keep them safe");
			} else if (branchMenu == 1) {
				Functions.npcTalk(player, npc, "Yes we did, but people kept on coming into our branches outside of varrock",
	...
	public void onNpcCommand(Npc n, String command, Player p) {
		if(Functions.inArray(n.getID(), BANKERS)) {
	...
	public boolean blockNpcCommand(Npc n, String command, Player p) {
		if(Functions.inArray(n.getID(), BANKERS) && command.equalsIgnoreCase("Bank")) {
			return true;
		}
		if(Functions.inArray(n.getID(), BANKERS) && command.equalsIgnoreCase("Collect")) {
			return true;
		}
		return false;
	}
