...
public class SidneySmith implements TalkToNpcListener, TalkToNpcExecutiveListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener {
	...
	private void sidneyCert(Player p, Npc n, int cID) {
		if(cID == -1) {
			Functions.npcTalk(p,n, "Hello, I'm Sidney Smith, the certification Clerk.",
					"How can I help you ?");
		...
		case Sidney.WHICH_GOODS_DO_YOU_CERTIFICATE:
			Functions.npcTalk(p,n, "Well, I can certificate the following items.",
					...
					"Dragon Bones,",
					"and Limpwurt Root.");
			int SUB_MENU_ONE = Functions.showMenu(p,n,
		...
		case Sidney.WHAT_IS_CERTIFICATION:
			Functions.npcTalk(p,n, "It's quite easy really..",
			...
			int SUB_MENU_TWO = Functions.showMenu(p,n,
					"I'd like to certificate some goods please.",
					"I'd like to change some certificates for goods please.",
					"Ok thanks.");
			if(SUB_MENU_TWO == 0) {
		...
		case Sidney.HOW_MANY_ITEMS_TO_MAKE_CERTIFICATE:
			Functions.npcTalk(p,n, "Well, you need at the least five items to make a certificate.",
					"We'll turn any five items into one certificate.",
					"It makes storage and transportation much easier.");
			int SUB_MENU_THREE = Functions.showMenu(p,n,
					"Which goods do you certificate?",
					"Ok, thanks.");
			if(SUB_MENU_THREE == 0) {
				sidneyCert(p, n, Sidney.WHICH_GOODS_DO_YOU_CERTIFICATE);
		...
		case Sidney.CERTIFICATE_TO_GOODS:
			if(!Functions.hasItem(p, PRAYER_CERT) &&
					!Functions.hasItem(p, SUPER_ATTACK_CERT) &&
					!Functions.hasItem(p, SUPER_DEFENSE_CERT) &&
					!Functions.hasItem(p, SUPER_STRENGTH_CERT) &&
					!Functions.hasItem(p, DRAGON_BONES_CERT) &&
					!Functions.hasItem(p, LIMPWURT_ROOT_CERT)) {
				Functions.npcTalk(p,n, "Sorry, but you don't have any certificates that I can change.",
				...
				Functions.npcTalk(p,n, "Ok then, which certificates would you like to change?");
				certMenuOne(p, n);
			}
			break;
		case Sidney.GOODS_TO_CERTIFICATE:
			if(!Functions.hasItem(p, PRAYER_RESTORE_POT, 5) &&
					!Functions.hasItem(p, SUPER_ATTACK_POT, 5) &&
					!Functions.hasItem(p, SUPER_DEFENSE_POT, 5) &&
					!Functions.hasItem(p, SUPER_STRENGTH_POT, 5) &&
					!Functions.hasItem(p, DRAGON_BONES, 5) &&
					!Functions.hasItem(p, LIMPWURT_ROOT, 5)) {
				Functions.npcTalk(p,n, "Sorry, but you either don't have enough items for me to certificate.",
						"or you don't have the right type of items for me to certificate.");
				int SUB_MENU_FOUR = Functions.showMenu(p,n,
						"Which goods do you certificate?",
						"How many items do you need to make a certificate.");
				if(SUB_MENU_FOUR == 0) {
					sidneyCert(p, n, Sidney.WHICH_GOODS_DO_YOU_CERTIFICATE);
				...
				Functions.npcTalk(p,n, "Which goods would you like to certificate?");
				goodsMenuOne(p, n);
			}
			break;
		}
	...
	private void goodsMenuOne(Player p, Npc n) {
		int goods = Functions.showMenu(p,
				"* Prayer Restore Potion * ",
				"* Super Attack Potion *",
				"* Super Defense Potion *",
				"* Super Strength Potion *",
		...
		if(goods == 0) {
			if(Functions.hasItem(p, PRAYER_RESTORE_POT, 5)) {
				...
				Functions.npcTalk(p,n, "You don't have any Prayer potions to certificate.",
						"Which goods would you like to certificate?");
				goodsMenuOne(p, n);
			}
		} else if(goods == 1) {
			if(Functions.hasItem(p, SUPER_ATTACK_POT, 5)) {
				calculateExchangeMenu(p, n, false, new Item(SUPER_ATTACK_POT), new Item(SUPER_ATTACK_CERT));
			} else {
				Functions.npcTalk(p,n, "You don't have enough Super Attack potions to certificate.");
				Functions.playerTalk(p,n, "Ok thanks.");
			}
		} else if(goods == 2) {
			if(Functions.hasItem(p, SUPER_DEFENSE_POT, 5)) {
				calculateExchangeMenu(p, n, false, new Item(SUPER_DEFENSE_POT), new Item(SUPER_DEFENSE_CERT));
			} else {
				Functions.npcTalk(p,n, "You don't have any Super Defense potions to certificate.",
						"Which goods would you like to certificate?");
				goodsMenuOne(p, n);
			}
		} else if(goods == 3) {
			if(Functions.hasItem(p, SUPER_STRENGTH_POT, 5)) {
				calculateExchangeMenu(p, n, false, new Item(SUPER_STRENGTH_POT), new Item(SUPER_STRENGTH_CERT));
			} else {
				Functions.npcTalk(p,n, "You don't have any Super Strength potions to certificate.",
						"Which goods would you like to certificate?");
				goodsMenuOne(p, n);
			}
		} else if(goods == 4) {
	...
	private void goodsMenuTwo(Player p, Npc n) {
		int goods = Functions.showMenu(p,
				...
				"-*- Menu 1 -*-");
		if(goods == 0) {
			if(Functions.hasItem(p, DRAGON_BONES, 5)) {
				calculateExchangeMenu(p, n, false, new Item(DRAGON_BONES), new Item(DRAGON_BONES_CERT));
			} else {
				Functions.npcTalk(p,n, "You don't have any Dragon Bones to certificate.",
						"Which goods would you like to certificate?");
				goodsMenuOne(p, n);
			}
		} else if(goods == 1) {
			if(Functions.hasItem(p, LIMPWURT_ROOT, 5)) {
				calculateExchangeMenu(p, n, false, new Item(LIMPWURT_ROOT), new Item(LIMPWURT_ROOT_CERT));
			} else {
				Functions.npcTalk(p,n, "You don't have any Limpwurt Roots to certificate.",
						"Which goods would you like to certificate?");
				goodsMenuOne(p, n);
			}
		} else if(goods == 2) {
	...
	private void certMenuOne(Player p, Npc n) {
		int certs = Functions.showMenu(p,
		...
		if(certs == 0) {
			if(Functions.hasItem(p, PRAYER_CERT)) {
				...
				Functions.npcTalk(p,n, "Sorry, but you don't have any ",
						"Prayer Restore Potion Certificates to change.");
			}
		} else if(certs == 1) {
			if(Functions.hasItem(p, SUPER_ATTACK_CERT)) {
			...
			if(Functions.hasItem(p, SUPER_DEFENSE_CERT)) {
			...
			if(Functions.hasItem(p, SUPER_STRENGTH_CERT)) {
	...
	private void certMenuTwo(Player p, Npc n) {
		int menu = Functions.showMenu(p,
				...
				"-*- Menu 1 -*-");
		if(menu == 0) {
			if(Functions.hasItem(p, DRAGON_BONES_CERT)) {
			...
			if(Functions.hasItem(p, LIMPWURT_ROOT_CERT)) {
	...
	private void calculateExchangeMenu(Player p, Npc n, boolean useCertificate, Item i, Item certificate) {
		...
		int mainMenu = -1;
		if(useCertificate) {
			Functions.npcTalk(p, n, "How many " + i.getDef().getName() + " certificates do you want to change?");
			if(count == 1) {
				int firstMenu = Functions.showMenu(p, "None thanks.", "One Certificate please");
				if(firstMenu != -1) {
					if(firstMenu == 0) {
				...
				mainMenu = Functions.showMenu(p, "One Certificate please", "Two Certificates Please");
			} else if(count == 3) {
				mainMenu = Functions.showMenu(p, "One Certificate please", "Two Certificates Please", "Three Certificates Please.");
			} else if(count == 4) {
				mainMenu = Functions.showMenu(p, "One Certificate please", "Two Certificates Please", "Three Certificates Please.", "Four Certificates Please");
			} else if(count >= 5) {
				mainMenu = Functions.showMenu(p, "One Certificate please", "Two Certificates Please", "Three Certificates Please.", "Four Certificates Please", "Five Certificates Please.");
			} 

		} else {
			Functions.npcTalk(p, n, "How many " + i.getDef().getName() + " would you like to certificate?");
			if(count >= 5 && count < 10) {
				int firstMenu = Functions.showMenu(p, "None", "Five");
				if(firstMenu != -1) {
					if(firstMenu == 0) {
				...
				mainMenu = Functions.showMenu(p, "Five", "Ten");
			} else if(count >= 15 && count < 20) {
				mainMenu = Functions.showMenu(p, "Five", "Ten", "Fifteen");
			} else if(count >= 20 && count < 25) {
				mainMenu = Functions.showMenu(p, "Five", "Ten", "Fifteen", "Twenty");
			} else if(count >= 25) {
				mainMenu = Functions.showMenu(p, "Five", "Ten", "Fifteen", "Twenty" , "Twenty Five");
			} else {
				Functions.npcTalk(p,n, "Sorry, but you don't have enough " + i.getDef().getName() + ".",
		...
		if(mainMenu != -1) {
			if(useCertificate) {
				Functions.npcTalk(p,n, "Ok, that's your " + i.getDef().getName() + " certificates done.");
				mainMenu += 1;
				...
				Functions.npcTalk(p,n, "Ok, that's your " + i.getDef().getName() + " certificated.");
				mainMenu += 1;
				int itemAmount = mainMenu * 5;
				for (int x = 0; x < itemAmount; x++) {
	...
	public boolean blockInvUseOnNpc(Player player, Npc npc, Item item) {
		return npc.getID() == SIDNEY_SMITH && Functions.inArray(item.getID(), PRAYER_RESTORE_POT,
	...
	public void onInvUseOnNpc(Player player, Npc npc, Item item) {
		if(npc.getID() == SIDNEY_SMITH && Functions.inArray(item.getID(), PRAYER_RESTORE_POT,
				SUPER_ATTACK_POT, SUPER_STRENGTH_POT, SUPER_DEFENSE_POT, DRAGON_BONES,
				LIMPWURT_ROOT, PRAYER_CERT, SUPER_ATTACK_CERT, SUPER_DEFENSE_CERT,
				SUPER_STRENGTH_CERT, DRAGON_BONES_CERT, LIMPWURT_ROOT_CERT)) {
			switch(item.getID()) {
