...
public class BrotherGalahad implements TalkToNpcExecutiveListener, TalkToNpcListener {
	...
	public void onTalkToNpc(final Player p, final Npc n) {
		if(p.getQuestStage(Constants.Quests.THE_HOLY_GRAIL) == -1) {
			Functions.npcTalk(p,n, "would you like a cup of tea?",
					"I'll just put the kettle on");
			p.message("Brother galahad hangs a kettle over the fire");
			Functions.playerTalk(p,n, "I returned the holy grail to camelot");
			Functions.npcTalk(p,n, "I'm impressed",
			...
			Functions.addItem(p, 739, 1);
			return;
		}
		Functions.npcTalk(p,n, "Welcome to my home",
		...
		defaultMenu.addOption(new Option("Are you any relation to Sir Galahad?") {
			@Override
			public void action() {
				Functions.npcTalk(p,n, "I am Sir Galahad",
		...
		if (p.getQuestStage(Constants.Quests.THE_HOLY_GRAIL) >= 2) {
			defaultMenu.addOption(new Option("I'm on a quest to find the holy grail") {
				@Override
				public void action() {
					Functions.npcTalk(p,n, "Ah the grail yes",
		...
		defaultMenu.addOption(new Option("do you get lonely here on your own?") {
			@Override
			public void action() {
				Functions.npcTalk(p,n, "Sometimes I do yes",
	...
	private void FIND_IT(Player p, Npc n) {
		Functions.npcTalk(p,n, "I did not find it through looking",
	...
	private void LEAVE(Player p, Npc n) {
		Functions.npcTalk(p,n, "apparently the time is getting close",
				"When the world will need Arthur and his knights of the round table again",
				"And that includes me",
				"leaving was tough for me",
				"I took this small cloth from the table as a keepsake");
		if(!Functions.hasItem(p, 742, 1) && p.getQuestStage(Constants.Quests.THE_HOLY_GRAIL) >= 3) {
			Functions.playerTalk(p,n, "I don't suppose I could borrow that?",
					"it could come in useful on my quest");
			p.message("Galahad reluctantly passes you a small cloth");
			Functions.addItem(p, 742, 1);
	...
	private void DIDNT_BRING(Player p, Npc n) {
		Functions.npcTalk(p,n, "I'm not sure",
	...
	private void TALKING_ABOUT(Player p, Npc n) {
		Functions.npcTalk(p,n, "The grail castle",
	...
	private void BETTER_BE_GOING(Player p, Npc n) {
		Functions.npcTalk(p,n, "Half a moment your cup of tea is ready");
		p.message("Sir Galahad gives you a cup of tea");
		Functions.addItem(p, 739, 1);
		Functions.npcTalk(p,n, "If you do come across any particularily difficult obstacles on your quest",
