...
public class ShantayPassNpcs implements ShopInterface,
	...
	public void onTalkToNpc(final Player p, Npc n) {
		if (n.getID() == SHANTAY_STANDING_GUARD) {
			Functions.npcTalk(p, n, "Hello there!", "What can I do for you?");
			int menu = Functions.showMenu(p, n, "I'd like to go into the desert please.",
					"Nothing thanks.");
			if (menu == 0) {
				Functions.npcTalk(p,n,"Of course!");
				if(!Functions.hasItem(p, SHANTAY_PASS)) {
					Functions.npcTalk(p, n, "You'll need a Shantay pass to go through the gate into the desert.",
							"See Shantay, he'll sell you one for a very reasonable price.");
				} else {
					int menus = 0;
					if(!Functions.hasItem(p, SHANTAY_DISCLAIMER)) {
						Functions.message(p, "There is a large poster on the wall near the gateway. It reads..",
						...
						menus = Functions.showMenu(p,
								"Yeah, that poster doesn't scare me!",
								"No, I'm having serious second thoughts now.");
					} else {
						Functions.message(p, "A poster on the wall says exactly the same as the disclaimer.",
					...
					if(menus == 0) {
						...
						Server.getServer().getEventHandler().add(
								new SingleEvent(null, 60000) {
									public void action() {
										Functions.npcTalk(p, npc, "Right, time for dinner!");
										npc.remove();
									}
								});
						Functions.sleep(1000);
						Functions.npcTalk(p,npc, "Can I see your Shantay Desert Pass please.");
						p.message("You hand over a Shantay Pass.");
						Functions.removeItem(p, SHANTAY_PASS, 1);
						Functions.playerTalk(p,npc, "Sure, here you go!");
						if(!Functions.hasItem(p, SHANTAY_DISCLAIMER)) {
							Functions.npcTalk(p, npc, "Here, have a disclaimer...",
									"It means that Shantay isn't responsible if you die in the desert.");
							p.message("The guard gives you a disclaimer.");
							Functions.addItem(p, SHANTAY_DISCLAIMER, 1);
						}
						p.message("you go through the gate");
						p.teleport(62, 735);
					} else if(menus == 1) {
						Functions.message(p, "You decide that your visit to the desert can be postponed..");
						p.message("Perhaps indefinitely!");
					}

				}
			} else if (menu == 1) {
				Functions.npcTalk(p, n, "Ok then, have a nice day.");
			}
			return;
		}
		if (n.getID() == SHANTAY_MOVING_GUARD) {
			Functions.npcTalk(p, n, "Go talk to Shantay or one of his assistants.",
					"I'm on duty and I don't have time to talk to the likes of you!");
			Functions.message(p, "The guard seems quite bad tempered,",
					"probably from having to wear heavy armour in this intense heat.");
			return;
		}
		if (n.getID() == SHANTAY) {
			Functions.npcTalk(p,n,"Hello Effendi, I am Shantay.");
			if(!Functions.hasItem(p, SHANTAY_DISCLAIMER)) {
				Functions.npcTalk(p,n, "I see you're new!",
						"Make sure you read the poster before going into the desert.");
			} 
			if(p.getQuestStage(Constants.Quests.TOURIST_TRAP) == 0) {
				Functions.npcTalk(p, n, "There is a heartbroken Mother just past the gates and in the Desert.",
						"Her name is Irena and she mourns her lost Daughter. Such a shame.");
			}
		} else if (n.getID() == ASSISTANT) {
			Functions.npcTalk(p, n, "Hello Effendi, I am a Shantay Pass Assistant.");
			if(!Functions.hasItem(p, SHANTAY_DISCLAIMER)) {
				Functions.npcTalk(p,n, "I see you're new!",
						"Make sure you read the poster before going into the desert.");
			}
		...
		if (menu == 0) {
			...
			int menu2 = Functions.showMenu(p, n,
					"I am definitely an outlaw, prepare to die!",
					"I am a little inexperienced.",
					"Er, neither, I'm an adventurer.");
			if (menu2 == 0) {
				Functions.npcTalk(p, n, "Ha, very funny.....", "Guards arrest him!");
				Functions.message(p, "The guards arrest you and place you in the jail.");
				Functions.npcTalk(p,
						n,
						"You'll have to stay in there until you pay the fine of five gold pieces.",
						"Do you want to pay now?");
				inJail = true;
				int menu6 = Functions.showMenu(p, n, "Yes, Ok.",
				...
				if (menu6 == 0) {
					...
					if (p.getInventory().countId(10) >= 5) {
						Functions.message(p, "You hand over five gold pieces to Shantay.");
						Functions.npcTalk(p, n,
						...
						int menu8 = Functions.showMenu(p, n, "I'll pay the fine.",
					...
					int menu7 = Functions.showMenu(p, n, "Yes, Ok.", "No, do your worst!");
					if (menu7 == 0) {
						Functions.npcTalk(p, n,
								"Good, I see that you have come to your senses.");
						if (p.getInventory().countId(10) >= 5) {
				...
				int menu5 = Functions.showMenu(p, n,
						"Can I see what you have to sell please?",
						"I must be going.");
				if (menu5 == 0) {
					Functions.npcTalk(p, n, "Absolutely Effendi!");
					p.setAccessingShop(shop);
					ActionSender.showShop(p,shop);
				} else if (menu5 == 1) {
					Functions.npcTalk(p, n, " So long...");
				}
			} else if (menu2 == 2) {
				Functions.npcTalk(p,
						n,
				...
				int menu3 = Functions.showMenu(p, n,
						"Can I see what you have to sell please?",
						"I must be going.",
				...
				if (menu3 == 0) {
					...
					int menu4 = Functions.showMenu(p, n,
							"Can I see what you have to sell please?",
							"I must be going.");
					if (menu4 == 0) {
	...
	public void onObjectAction(GameObject obj, String command, Player p) {
		if (obj.getID() == BANK_CHEST) {
			...
			if(!p.getCache().hasKey("shantay-chest")) {
				Functions.message(p, "This chest is used by Shantay and his men.",
		...
		if(obj.getID() == STONE_GATE) {
			if(command.equals("go through")) {
				if(p.getY() >= 735) {
					...
					Npc shantayGuard = Functions.getNearestNpc(p, SHANTAY_STANDING_GUARD, 5);
					if(menu == 0) {
						if(!Functions.hasItem(p, SHANTAY_PASS)) {
							Functions.message(p, "A guard stops you on your way out of the gate...");
							if(shantayGuard != null) {
								Functions.npcTalk(p,shantayGuard, "You need a Shantay pass to get through this gate.",
							...
							if(shantayGuard != null) {
								Functions.npcTalk(p,shantayGuard, "Can I see your Shantay Desert Pass please.");
								p.message("You hand over a Shantay Pass.");
								Functions.removeItem(p, SHANTAY_PASS, 1);
								Functions.playerTalk(p,shantayGuard, "Sure, here you go!");
								if(!Functions.hasItem(p, SHANTAY_DISCLAIMER)) {
									Functions.npcTalk(p, shantayGuard, "Here, have a disclaimer...",
											"It means that Shantay isn't responsible if you die in the desert.");
									p.message("The guard gives you a disclaimer.");
									Functions.addItem(p, SHANTAY_DISCLAIMER, 1);
								}
				...
				Functions.message(p, "You look at the huge Stone Gate.",
	...
	public void onPickup(Player p, GroundItem i) {
		if(i.getID() == 1099) {
			...
			Functions.addItem(p, 1099, 1);
			i.remove();
			int menu = Functions.showMenu(p, "Yes, I'll read it now!", "No thanks, it'll keep!");
			if(menu == 0) {
