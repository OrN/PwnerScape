...
public class Thrander implements TalkToNpcListener, TalkToNpcExecutiveListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener {
	...
	public void onTalkToNpc(Player p, Npc n) {
		Functions.npcTalk(p, n, "Hello I'm Thrander the smith",
	...
	public boolean blockInvUseOnNpc(Player player, Npc npc, Item item) {
		return npc.getID() == THRANDER && (getNewID(item) != -1 || Functions.inArray(item.getID(), 108, 6, 109, 230, 110, 111, 112, 2144, 2146, 2148, 2150, 2152, 2154, 2156,
	...
	public void onInvUseOnNpc(Player player, Npc npc, Item item) {
		if(Functions.inArray(item.getID(), 108, 6, 109, 230, 110, 111, 112, 2144, 2146, 2148, 2150, 2152, 2154, 2156,
				2158, 2160, 2162, 2164, 2166, 2168, 2170, 2172, 2174, 2176, 2178, 2180, 2182, 2184,
				2186, 2188, 2190, 2192, 2194, 2196, 2198, 2200, 2202, 2204, 2206, 2208, 2210, 2212,
				2214, 2216, 2218, 2220, 2222, 2224, 2226, 2228, 2230, 2232, 2234, 2236, 2238, 2240, 
				2242, 2244, 2246, 2248, 2250, 2252, 2254)) {
			Functions.npcTalk(player, npc, "Hello, changing your helm feather has a cost of 500 gold.",
					"Please pick a new color of your choice.");
			String[] menuText = { "Red (default)", "Orange", "Blue", "Purple", "Yellow", "Green", "Black", "White", "Matched" };
			int menu = Functions.showMenu(player, npc, menuText);
			int[] whatColor = null;
			if(menu == 0) {
			...
			if(menu != -1 && whatColor != null) {
				if(Functions.hasItem(player, 10, 500)) {
					Item newHelmet = null;
					int newHelmetArray = -1;
					if(Functions.inArray(item.getID(), BRONZE_LARGE_HELMETS))
						newHelmetArray = 0;
					else if(Functions.inArray(item.getID(), IRON_LARGE_HELMETS))
						newHelmetArray = 1;
					else if(Functions.inArray(item.getID(), STEEL_LARGE_HELMETS))
						newHelmetArray = 2;
					else if(Functions.inArray(item.getID(), BLACK_LARGE_HELMETS))
						newHelmetArray = 3;
					else if(Functions.inArray(item.getID(), MITHRIL_LARGE_HELMETS))
						newHelmetArray = 4;
					else if(Functions.inArray(item.getID(), ADAMANTITE_LARGE_HELMETS))
						newHelmetArray = 5;
					else if(Functions.inArray(item.getID(), RUNE_LARGE_HELMETS))
						newHelmetArray = 6;

					if(newHelmetArray != -1) {
						for(int i = 0; i < whatColor.length; i++) {
							if(i == newHelmetArray) {
								newHelmet = Functions.getItem(whatColor[i]);
								if(newHelmet == null) {
									Functions.npcTalk(player, npc, "Looks like I can't convert to that feather color");
								...
								if(Functions.removeItem(player, item.getID(), 1)) {
									Functions.removeItem(player, 10, 500);
									Functions.message(player, npc, 1300, "You give Thrander a " + item.getDef().getName().toLowerCase() + " and 500 coins",
											"Thrander hammers it for a bit");
									player.message("Thrander gives you a " + menuText[menu] + " " + newHelmet.getDef().getName().toLowerCase() + "");
									Functions.addItem(player, newHelmet.getID(), 1);
									return;
								}
							}
						} 
					...
					Functions.playerTalk(player, npc, "Oops, I don't have enough money");
				}
			} 
		}
		else if(Functions.inArray(item.getID(), 308, 312, 309, 313, 310, 311, 407, 117,
			...
			Item changedItem = Functions.getItem(newID);
			if(Functions.removeItem(player, item.getID(), 1)) {
				Functions.message(player, npc, 1300, "You give Thrander a " + item.getDef().getName().toLowerCase() + "",
						"Thrander hammers it for a bit");
