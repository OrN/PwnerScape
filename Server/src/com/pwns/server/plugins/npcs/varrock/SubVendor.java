...
public class SubVendor implements TalkToNpcExecutiveListener, TalkToNpcListener, PickupListener, PickupExecutiveListener, InvActionListener, InvActionExecutiveListener {
	...
	public void onTalkToNpc(Player p, Npc n) {
		if(n.getID() == SUB_VENDOR_FEMALE || n.getID() == SUB_VENDOR_MALE) {
			Functions.npcTalk(p, n, "Hello, I'm the subscription vendor",
					"How can I help you?");
			int menu = Functions.showMenu(p, n, "I want to exchange my gold token", "I want to exchange my premium token", "What are these tokens?", "I want to claim my teleport stones", "I want to change my death spawn location");
			if(menu == 0) {
				if(Functions.hasItem(p, GOLD_TOKEN)) {
					tokenExchangeTime(p, n, new Item(GOLD_TOKEN));
				} else {
					p.message("You don't have any gold tokens");
				}
			} else if(menu == 1) {
				if(Functions.hasItem(p, PREMIUM_TOKEN)) {
					tokenExchangeTime(p, n, new Item(PREMIUM_TOKEN));
				} else {
					p.message("You don't have any premium tokens");
				}
			} else if(menu == 2) {
				Functions.npcTalk(p, n, "There are two types of tokens",
				...
				if(p.getTeleportStones() > 0) {
					Functions.npcTalk(p, n, "Ok. You have " + p.getTeleportStones() + " stones to claim",
							"Do you want to claim them now?");
					int stoneMenu = Functions.showMenu(p, n, "Yes, I want to claim them now", "Not yet");
					if(stoneMenu == 0) {
						p.message("You claim your teleport stones...");
						Functions.addItem(p, 2107, p.getTeleportStones());
						p.setTeleportStones(0);
						Server.getPlayerDataProcessor().getDatabase().setTeleportStones(0, p.getOwner());
						Functions.npcTalk(p, n, "There you go, have a nice day!");
					}
				} else {
					Functions.npcTalk(p, n, "sorry, you have no teleport stones to claim");
				}
			} else if(menu == 4) {
				if(p.getDaysSubscriptionLeft() > 0) {
					Functions.npcTalk(p, n, "Yes " + (p.isMale() ? " Sir" : "Ma'am"));
					if(p.getCache().hasKey("death_location_x") && p.getCache().getInt("death_location_x") == 216) {
						Functions.npcTalk(p, n, "Your current death location is in Edgeville");
					} else {
						Functions.npcTalk(p, n, "Your current death location is in Lumbridge");
					}
					Functions.npcTalk(p, n, "Where would you like to change your death spawn to?",
							"The service is free of charge");
					int spawnMenu = Functions.showMenu(p, n, "Edgeville", "Lumbridge");
					if(spawnMenu == 0) {
					...
					Functions.npcTalk(p, n, "Sorry, " + (p.isMale() ? " Sir" : "Ma'am") + "!",
		...
		if(i.getID() == GOLD_TOKEN && i.getX() == 119 && i.getY() == 516) {
			ActionSender.sendBox(p,
					"@yel@Subscription Token %"
							...
							+ "www.pwnerscape.com/donate.php" , true);
		}
	}
	...
	private void tokenExchangeTime(Player p, Npc n, Item i) {
		...
		switch(i.getID()) {
		case GOLD_TOKEN:
			if(Functions.removeItem(p, i.getID(), 1)) {
				...
				if(n != null)
					Functions.npcTalk(p, n, "Thank you! Your membership will last for " + p.getDaysSubscriptionLeft() + " days");
		...
		case PREMIUM_TOKEN:
			if(p.getDaysSubscriptionLeft() > 0) {
				if(Functions.removeItem(p, i.getID(), 1)) {
					...
					Functions.npcTalk(p, n, "Thank you! Your premium membership will last for " + p.premiumSubDaysLeft() + " days");
				...
				Functions.npcTalk(p, n, "To gain maximum subscriber benefits",
	...
	public void onInvAction(Item item, Player player) {
		if(item.getID() == GOLD_TOKEN || item.getID() == PREMIUM_TOKEN) {
			Functions.message(player, "You can exchange this token for subscription time");
