...
public class StrangeBarrels implements ObjectActionListener, ObjectActionExecutiveListener {
		...
		if(obj.getID() == STRANGE_BARREL) {
			...
			if(action != 0) {
				...
				Server.getServer().getEventHandler().add(new SingleEvent(null, 40000) { // 40 seconds
					public void action() {
						...
						if(RegionManager.getRegion(Point.location(newObjectX, newObjectY)).getGameObject(Point.location(newObjectX, newObjectY)) != null) {
							Functions.registerObject(new GameObject(obj.getLoc()));
						} else {
							Functions.registerObject(new GameObject(Point.location(newObjectX, newObjectY), 1178, 0, 0));
						}
					}
				});
				/**
				...
				if(DataConversions.random(0, 1) != 1) {
					p.message("You smash the barrel open.");
					Functions.removeObject(obj);
					Functions.delayedSpawnObject(obj.getLoc(), 40000); // 40 seconds
				} else {
					if(DataConversions.random(0, 1) != 0) {
						p.message("You were unable to smash this barrel open.");
						Functions.message(p, 1300, "You hit the barrel at the wrong angle.",
						...
						p.getSkills().setLevel(Functions.ATTACK, p.getSkills().getLevel(Functions.ATTACK) - reduceAttack);
					} else {
						p.message("You were unable to smash this barrel open.");
					}
				} 
	...
	private void spawnMonster(Player p, int x, int y) {
		...
		Npc monster = Functions.spawnNpc(selectedMonster, x, y, 60000 * 3); // 3 minutes
		Functions.sleep(600);
		if(monster != null) {
