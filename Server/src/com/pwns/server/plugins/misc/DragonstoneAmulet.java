...
public class DragonstoneAmulet implements InvActionListener, InvActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
	...
	public void onInvAction(Item item, Player p) {
		if(item.getID() == CHARGED_DRAGONSTONE_AMULET) {
			...
			int menu = Functions.showMenu(p, "Edgeville", "Karamja", "Draynor village", "Al Kharid", "Seers", "Yanille", "Nowhere");
	...
	public void onInvUseOnObject(GameObject obj, Item item, Player p) {
		if(obj.getID() == FOUNTAIN_OF_HEROES && item.getID() == DRAGONSTONE_AMULET) {
			...
			p.setBatchEvent(new BatchEvent(p, 650, Formulae.getRepeatTimes(p, Functions.CRAFTING)) {
				@Override
				public void action() {
			...
			Functions.message(p, "You feel more power emanating from it than before",
