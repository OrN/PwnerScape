...
public class RandomObjects implements ObjectActionExecutiveListener,
	...
	public void onObjectAction(final GameObject object, String command,
		...
		/* Brimhaven Jungle tree swings TODO: Possibly should give agility XP ? */
			/* Stones */
		case 701:
			Functions.message(owner, "You jump onto the rock");
			...
			Functions.message(owner, "And cross the water without problems");
			break;
		case 79:
			if(command.equals("close") && object.getID() == 79) {
				owner.setBusyTimer(600);
				owner.message("You slide the cover back over the manhole");
				Functions.replaceObject(object, new GameObject(object.getLocation(), 78, object.getDirection(), object.getType()));
			} else {
		...
		case 78:
			if(command.equals("open") && object.getID() == 78) {
				...
				Functions.replaceObject(object, new GameObject(object.getLocation(), 79, object.getDirection(), object.getType()));
			}
			break;
		case 203:
		...
		case 613: // Shilo cart
			...
			if(owner.getX() >= 386) {
				Functions.message(owner, "You climb up onto the cart.",
			...
			if(owner.getQuestStage(Constants.Quests.SHILO_VILLAGE) != -1) {
				Functions.message(owner, "You approach the cart and see undead creatures gathering by the village gates.",
						"There is a note attached to the cart.",
						"The note says,",
						"@gre@Danger deadly green mist do not enter if you value your life");
				Npc mosol = Functions.getNearestNpc(owner, 539, 15);
				if(mosol != null) {
					Functions.npcTalk(owner, mosol, "You must be a maniac to go in there!");
				}
			} else {
				owner.message("It looks as if you can climb across.");
			}
			Functions.message(owner, "You search the cart.",
					"You may be able to climb across the cart.",
					"Would you like to try?");
			int menu = Functions.showMenu(owner,
			...
			if(menu == 0) {
				...
				Functions.message(owner, "You think better of clambering over the cart, you might get dirty.");
				Functions.playerTalk(owner, null, "I'd probably have just scraped my knees up as well.");
			}
			break;
		...
		case 242:
		case 243:
			Functions.message(owner, "You board the ship");
		...
		if(object.getID() == 450) {
			Functions.message(owner, "you pull on the large wooden doors");
			if(owner.getQuestStage(Constants.Quests.BIOHAZARD) == -1) {
				owner.message("you open it and walk through");
				Npc gateMourner = Functions.getNearestNpc(owner, 451, 15);
				if(gateMourner != null) {
					Functions.npcTalk(owner, gateMourner, "go through");
				}
				if(owner.getX() >= 624) {
		...
		if(object.getID() == 400) {
			...
			owner.damage(Functions.getCurrentLevel(owner, Functions.HITS) / 10 + 2);
		}
		return;
	}
