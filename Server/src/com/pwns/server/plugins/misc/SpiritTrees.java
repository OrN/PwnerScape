	...
	public void onObjectAction(GameObject obj, String command, Player p) {
		...
		if(obj.getID() == STRONGHOLD_SPIRIT_TREE) {
			if(p.getQuestStage(Constants.Quests.GRAND_TREE) == -1) {
				Functions.message(p, "The tree talks in an old tired voice...",
						"@yel@Spirit Tree: You friend of gnome people, you friend of mine",
						"@yel@Spirit Tree: Would you like me to take you somewhere?");
				int treeMenu = Functions.showMenu(p, "No thanks old tree",
						"Where can i go?");
				if (treeMenu == 0) {
					Functions.playerTalk(p, null, "no thanks old tree");
				} else if (treeMenu == 1) {
					Functions.playerTalk(p, null, "where can i go?");
					Functions.message(p, "The tree talks again..",
							"@yel@Spirit Tree: You can travel to the trees",
							"@yel@Spirit Tree: Which are related to myself");
					int travelMenu = Functions.showMenu(p,
		...
		/** Battle field of Khazard tree **/
			...
			if (p.getQuestStage(Constants.Quests.TREE_GNOME_VILLAGE) == -1 || p.getQuestStage(Constants.Quests.GRAND_TREE) == -1) {
				...
				Functions.message(p, "@yel@Young Spirit Tree: Hello gnome friend",
						"@yel@Young Spirit Tree: Would you like to travel to the home of the tree gnomes?");
				int treeMenu = Functions.showMenu(p, "No thank you",
						"Yes please");
				if(treeMenu == 0) {
					Functions.playerTalk(p, null, "No thank you");
				} else if(treeMenu == 1) {
					Functions.playerTalk(p, null, "Yes please");
					Functions.message(p, "You place your hands on the dry tough bark of the spirit tree",
							"and feel a surge of energy run through your veins");
					breakPlagueSample(p);
