...
public class Panning implements ObjectActionListener, ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener, InvUseOnNpcListener, InvUseOnNpcExecutiveListener, InvActionListener, InvActionExecutiveListener {
	...
	private boolean handlePanning(GameObject obj, Item item, Player p) {
		...
		p.setBusy(true);
		Functions.showBubble(p, new Item(PANNING_TRAY));
		p.playerServerMessage(MessageType.QUEST, "You scrape the tray along the bottom");
		Functions.message(p, "You swirl away the excess water");
		Functions.sleep(1500);
		Functions.showBubble(p, new Item(FULL_PANNING_TRAY));
		...
		p.incExp(Functions.MINING, 5.0, true);
		p.setBusy(false);
		return false;
	}
	...
	public void onInvUseOnObject(GameObject obj, Item item, Player p) {
		if(obj.getID() == PANNING_POINT) {
			if(item.getID() == PANNING_TRAY) {
				Npc guide = Functions.getNearestNpc(p, GUIDE, 15);
				if(guide != null) {
					// NOT SURE? if(p.getQuestStage(Constants.Quests.DIGSITE) < 2) {
					if(!p.getCache().hasKey("unlocked_panning")) {
						Functions.npcTalk(p, guide, "Hey! you can't pan yet!");
						Functions.playerTalk(p, guide, "Why not ?");
						Functions.npcTalk(p, guide, "We do not allow the uninvited to pan here");
						int menu = Functions.showMenu(p, guide,
								"Okay, forget it",
								"So how do I become invited then ?");
						if(menu == 0) {
							Functions.npcTalk(p, guide, "You can of course use this place when you know what you are doing");
						} else if(menu == 1) {
							Functions.npcTalk(p, guide, "I'm not supposed to let people pan here",
							...
							Functions.playerTalk(p, guide, "What's that ?");
							Functions.npcTalk(p, guide, "Well...to be honest...",
									"What I would really like...",
									"Is a nice cup of tea !");
							Functions.playerTalk(p, guide, "Tea !?");
							Functions.npcTalk(p, guide, "Absolutely, I'm parched !",
	...
	public void onInvUseOnNpc(Player p, Npc npc, Item item) {
		if(npc.getID() == GUIDE) {
			if(item.getID() == PANNING_TRAY) {
				...
				Functions.npcTalk(p, npc, "Yes, this is a panning tray...");
			}
			if(item.getID() == FULL_PANNING_TRAY) {
				p.message("You give the full panning tray to the guide");
				Functions.npcTalk(p, npc, "This is no good to me",
						"I don't deal with finds");
			}
			if(item.getID() == GOLD_NUGGET_TRAY) {
				p.message("You give the full panning tray to the guide");
				Functions.npcTalk(p, npc, "I am afraid I don't deal with finds",
						"That's not my job");
			}
			if(item.getID() == CUP_OF_TEA) {
				if(p.getCache().hasKey("unlocked_panning")) {
					Functions.npcTalk(p, npc, "No thanks, I've had enough!");
				} else {
					Functions.npcTalk(p, npc, "Ah! Lovely!",
							"You can't beat a good cuppa...",
							"You're free to pan all you want");
					Functions.playerTalk(p, npc, "Thanks");
					Functions.removeItem(p, CUP_OF_TEA, 1);
	...
	public void onInvAction(Item item, Player p) {
		if(item.getID() == PANNING_TRAY) {
			...
			Functions.playerTalk(p, null, "Err, why am I searching an empty tray ?");
		}
		if(item.getID() == FULL_PANNING_TRAY) {
			p.setBusy(true);
			Functions.message(p, "You search the contents of the tray...");
			Functions.sleep(1500);
		...
		if(item.getID() == GOLD_NUGGET_TRAY) {
			...
			Functions.addItem(p, GOLD_NUGGET, 1);
