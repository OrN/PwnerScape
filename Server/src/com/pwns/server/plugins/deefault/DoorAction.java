...
import com.pwns.server.Constants.GameServer;
import com.pwns.server.Server;
import com.pwns.server.event.ShortEvent;
...
public class DoorAction {
	...
	public void onWallObjectAction(final GameObject obj, final Integer click,
		...
		case 54: /* Dragon Slayer: Door out of maze near entrace*/
			...
			break;
		case 154:
			Functions.message(p, 1900, "you open the door");
			p.teleport(703, 455);
		...
		case 74:
			if (p.getQuestStage(Constants.Quests.HEROS_QUEST) == -1) {
				...
				Npc achetties = Functions.getNearestNpc(p, 253, 10);
				...
				case 1:
				case 2:
					Functions.npcTalk(p, achetties,
							"Greetings welcome to the hero's guild",
							"How goes thy quest?");
					if (Functions.hasItem(p, 586) && Functions.hasItem(p, 590) && Functions.hasItem(p, 557)) {
						Functions.playerTalk(p, achetties, "I have all the things needed");
						p.sendQuestComplete(Constants.Quests.HEROS_QUEST);
					} else {
						Functions.playerTalk(p, achetties,
						...
						int opt2 = Functions.showMenu(p, achetties,
		...
		case 109:
			if(p.getQuestStage(Constants.Quests.TEMPLE_OF_IKOV) >= 1 || p.getQuestStage(Constants.Quests.TEMPLE_OF_IKOV) == -1 || p.getQuestStage(Constants.Quests.TEMPLE_OF_IKOV) == -2) {
				...
				Functions.message(p, "The door doesn't open",
		...
		case 108:
			if(p.getCache().hasKey("killedLesarkus") || p.getQuestStage(Constants.Quests.TEMPLE_OF_IKOV) == -1 || p.getQuestStage(Constants.Quests.TEMPLE_OF_IKOV) == -2) {
				...
				Functions.message(p, "The fire warrior's eyes glow",
		...
		case 104:
			if(p.getInventory().wielding(721) ||  p.getY() >= 3335 && p.getY() <= 3341) {
				...
				Functions.message(p, "As you reach to open the door",
		...
		case 113:
			if (p.getInventory().wielding(733)
				...
				Npc guardfinal = Functions.getNearestNpc(p, 373, 8);
				if (guardfinal != null) {
					Functions.npcTalk(p, guardfinal, "you there! halt!",
							"this is General Khazard's private lodgings",
							"now leave and don't return!");
				}
				Npc guard = Functions.getNearestNpc(p, 374, 8);
				if (guard != null) {
					Functions.npcTalk(p, guard, "you there! halt!",
							"this is General Khazard's private lodgings",
							"now leave and don't return!");
				}
			}
		...
		case 114:
			...
			Npc guard385 = Functions.getNearestNpc(p, 385, 8);
			if (guard385 != null) {
				Functions.npcTalk(p, guard385, "and where do you think you're going?",
		...
		case 122:
			...
			Npc ted = Functions.getNearestNpc(p, 446, 8);
			if (ted != null) {
				p.message("The door won't open");
				Functions.npcTalk(p, ted, "Go away we don't want any");
				if (p.getY() >= 569) {
					if (Functions.removeItem(p, 768,1)) {
						Functions.playerTalk(p, ted,
								"I have come to return a book from Jethick");
						Functions.npcTalk(p, ted, "Ok I guess you can come in then");
						Functions.doDoor(obj, p);
			...
			break;
		case 123:
			Npc mourner = Functions.getNearestNpc(p, 445, 8);
			...
			if (p.getY() <= 605 || p.getY() >= 612) {
				...
				if (mourner != null) {
					Functions.npcTalk(p, mourner, "I'd stand away from there",
							"That black cross means that house has been touched by the plague");
					if (Functions.hasItem(p, 775)) {
						Functions.playerTalk(p, mourner,
								"I have a warrant from Bravek to enter here");
						Functions.npcTalk(p, mourner, "this is highly irregular",
					...
					if (p.getQuestStage(Constants.Quests.PLAGUE_CITY) == 7) {
						int menu = Functions.showMenu(p, mourner,
								"but I think a kidnap victim is in here",
								"I fear not a mere plague",
								"thanks for the warning");
						if (menu == 0) {
							Functions.npcTalk(p, mourner, "Sounds unlikely",
									"Even kidnappers wouldn't go in there",
									"even if someone is in there",
									"They're probably dead by now");
							int menu2 = Functions.showMenu(p, mourner, "Good point",
									"I want to check anyway");
							if (menu2 == 0) {
								// NOTHING
							} else if (menu2 == 1) {
								Functions.npcTalk(p, mourner,
							...
							Functions.npcTalk(p, mourner, "that's irrelevant",
									"You don't have clearance to go in there");
							Functions.playerTalk(p, mourner, "How do I get clearance?");
							Functions.npcTalk(p,
									mourner,
									"Well you'd need to apply to the head mourner",
									"Or I suppose Bravek the city warder",
			...
			break;
		case 121:
			Npc Bravek = Functions.getNearestNpc(p, 445, 8);
			...
			if (Bravek != null) {
				Functions.npcTalk(p, Bravek, "Go away,I'm busy", "I'm", "um",
						"In a meeting");
				p.message("The door won't open");
			}
			break;
			...
			if (p.getY() > 523) {
				if (p.getSkills().getLevel(10) < 68) {
					...
					if (masterFisher != null) {
						Functions.npcTalk(p, masterFisher, "Hello only the top fishers are allowed in here");
					}
		...
		case 55: // Mining Guild Door
			...
			if (p.getSkills().getLevel(14) < 60) {
				...
				if (dwarf != null) {
					Functions.npcTalk(p, dwarf, "Hello only the top miners are allowed in here");
				}
				Functions.sleep(600);
		...
		case 68: // Crafting Guild Door
			...
			if (p.getSkills().getLevel(12) < 40) {
				...
				if (master != null) {
					Functions.npcTalk(p, master, "Hello only the top crafters are allowed in here");
				}
				Functions.sleep(600);
				p.setBusy(false);
				...
				Npc master = World.getWorld().getNpc(231, 341, 349, 599, 612);
				if (master != null) {
					Functions.npcTalk(p, master, "Where's your brown apron?",
		...
		case 43: // Cooking Guild Door
			...
			if (p.getSkills().getLevel(7) < 32) {
				...
				if (chef != null) {
					Functions.npcTalk(p, chef, "Hello only the top cooks are allowed in here");
				}
				Functions.sleep(600);
				...
				Npc chef = World.getWorld().getNpc(133, 176, 181, 480, 487);
				if (chef != null) {
					Functions.npcTalk(p, chef,"Where's your chef's hat",
		...
		case 146: // Magic Guild Door
			...
			if (p.getSkills().getLevel(6) < 66) {
				...
				if (wizard != null) {
					Functions.npcTalk(p, wizard, "You need a magic level of 66 to get in here",
		...
		case 44: // champs guild door
			...
			if (p.getQuestPoints() < 32) {
				final Npc champy = Functions.getNearestNpc(p, 111, 20);
				if (champy != null) {
					Functions.npcTalk(p, champy,
		...
		case 67:
			...
			if (n != null) {
				...
				Functions.playerTalk(p, n, "What do I need to pay?");
				Functions.npcTalk(p, n, "One diamond");
				int m = Functions.showMenu(p, n, "Okay", "A diamond, are you crazy?",
						"I haven't brought my diamonds with me");
				if (m == 0) {
					if (!Functions.hasItem(p, 161)) {
						Functions.playerTalk(p, n,
								"I haven't brought my diamonds with me");
					} else {
						p.message("You give the doorman a diamond");
						Functions.removeItem(p, 161, 1);
						Functions.doDoor(obj, p);
					}
				}
				else if (m == 1) {
					Functions.npcTalk(p, n, "Nope those are the rules");
				}
			}
			break;
		...
		case 138:
			if(!p.getCache().hasKey("rotten_apples") && p.getQuestStage(Constants.Quests.BIOHAZARD) == 4) {
				Functions.message(p, "the door is locked",
				...
				Npc DOOR_MOURNER = Functions.getNearestNpc(p, 492, 10);
				if(DOOR_MOURNER != null) {
					if(p.getInventory().wielding(802)) {
						Functions.npcTalk(p,DOOR_MOURNER, "in you go doc");
						Functions.doDoor(obj, p);
						p.message("You go through the door");
					} else {
						Functions.npcTalk(p,DOOR_MOURNER, "keep away from there");
						Functions.playerTalk(p,DOOR_MOURNER, "why?");
						Functions.npcTalk(p,DOOR_MOURNER, "several mourners are ill with food poisoning",
	...
	private void handleGates(GameObject obj, Player player) {
		...
		case 660:
			...
			Functions.doGate(player, obj, 357);
			Functions.sleep(1000);
			...
			if (player.getY() <= 472) {
				...
				if (player.getSkills().getLevel(8) < 70) {
					...
					final Npc mcgrubor = World.getWorld().getNpc(255, 556, 564,
							473, 476);
					if (mcgrubor != null) {
						Functions.npcTalk(player,mcgrubor, "Hello only the top woodcutters are allowed in here");
					}
					Server.getServer().getEventHandler().add(
		...
		case 712: // Shilo inside gate
			...
			if(player.getX() >= 394) {
				Functions.message(player, "The gate opens smoothly");
		...
		case 611: // Shilo outside gate
			...
			Functions.message(player, "The gate feels very cold to your touch!");
			player.message("Are you sure you want to go through?");
			int menu = Functions.showMenu(player,
					"Yes, I am fearless!",
					"No, actually, I have a bad feeling about this!");
			if(menu == 0) {
				Functions.replaceObjectDelayed(obj, 3000, 612);
				Functions.message(player, "The gates open very slowly...");
				if(player.getX() >= 388) {
					...
					Functions.message(player, "As soon as the gates open, the Zombies grab you and start dragging you inside!");
					player.teleport(391, 852);
					Functions.playerTalk(player, null, "Oh no, I'm done for!");
				}
			} else if(menu == 1) {
				Functions.message(player, "You drag your quivering body  away from the gates.");
		...
		case 513: // eastern Varrock gate for family crest or biohazard or just wanna go in :)
			...
			if (player.getX() >= 94) {
				...
				Functions.doGate(player, obj, 514);
			}
		...
		case 563: /* King Lanthlas Gate */
			Npc lathasGuard = Functions.getNearestNpc(player, 524, 10);
			if(player.getQuestStage(Constants.Quests.BIOHAZARD) == -1) {
				if(player.getY() <= 551) {
					Functions.doGate(player, obj);
				} else {
					if(lathasGuard != null) {
						Functions.npcTalk(player,lathasGuard, "the king has granted you access to this training area",
				...
				Functions.npcTalk(player,lathasGuard, "this is a restricted area",
		...
		case 626: /* Gnome Stronghold Gate */
			if(player.getY() != 532 && player.getQuestStage(Constants.Quests.GRAND_TREE) == 8) {
				Npc n = Functions.getNearestNpc(player, 562, 15);
				if(n != null) {
					Functions.npcTalk(player,n, "halt human");
					Functions.playerTalk(player,n, "what?, why?");
					Functions.npcTalk(player,n, "from order of the head tree guardian...",
							"..you cannot leave");
					Functions.playerTalk(player,n, "that's crazy, why?");
					Functions.npcTalk(player,n, "humans are planning to attack our stronghold",
							"you could be a spy");
					Functions.playerTalk(player,n, "that's ridiculous");
					Functions.npcTalk(player,n, "maybe, but that's the orders, I'm sorry");
					Functions.message(player, "the gnome refuses to open the gate");
				} else {
					Npc newN = Functions.spawnNpc(562, 705, 530, 30000);
					Functions.npcTalk(player,newN, "halt human");
					Functions.playerTalk(player,newN, "what?, why?");
					Functions.npcTalk(player,newN, "from order of the head tree guardian...",
							"..you cannot leave");
					Functions.playerTalk(player,newN, "that's crazy, why?");
					Functions.npcTalk(player,newN, "humans are planning to attack our stronghold",
							"you could be a spy");
					Functions.playerTalk(player,newN, "that's ridiculous");
					Functions.npcTalk(player,newN, "maybe, but that's the orders, I'm sorry");
					Functions.message(player, "the gnome refuses to open the gate");
					newN.remove();
				}
				return;
				...
				Npc n = Functions.getNearestNpc(player, 562, 15);
				Functions.npcTalk(player,n, "i'm afraid that we have orders not to let you in");
				Functions.playerTalk(player,n, "orders from who?");
				Functions.npcTalk(player,n, "the head tree guardian, he say's you're a spy");
				Functions.playerTalk(player,n, "glough!");
				Functions.npcTalk(player,n, " i'm sorry but you'll have to leave");
				return;
			}
			members = true;
			break;
