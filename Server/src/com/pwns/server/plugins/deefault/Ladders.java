...
import com.pwns.server.model.TelePoint;
...
public class Ladders {
	...
	public void onObjectAction(GameObject obj, String command, Player player) {
		...
		if(obj.getID() == 79 && obj.getX() == 243 && obj.getY() == 95) {
			...
			int menu = Functions.showMenu(player,  "Yes I take the risk!", "No stay up here.");
			if(menu == 0) {
		...
		if(obj.getID() == 487) {
			...
			if (player.getSkills().getMaxStat(5) < 31) {
				...
				if (abbot != null) {
					...
					Functions.npcTalk(player, abbot, "Hello only people with high prayer are allowed in here");
				} else {
					return;
				}
			...
			if (player.getSkills().getLevel(14) < 60) {
				...
				if (dwarf != null) {
					Functions.npcYell(player, dwarf,
			...
			Npc paladinGuard = Functions.getNearestNpc(player, 323, 4);
			...
			if(ladderAttendant != null) {
				Functions.npcTalk(player, ladderAttendant, "This ladder leaves Zanaris",
				...
				int m = Functions.showMenu(player, ladderAttendant, "I think I'll stay down here a bit longer", "Yes, I'm ready to leave");
				if(m == 1) {
					player.message("You climb up the ladder");
