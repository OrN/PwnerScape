...
public class GnomeCooking implements InvActionListener, InvActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
	...
	private void handleGnomeCooking(final Item item, Player p, final GameObject object) {
		...
		if (p.getInventory().remove(item) > -1) {
			Functions.message(p, 3000, gc.messages[0]);
			if (!burnFood(p, gc.requiredLevel, p.getSkills().getLevel(Functions.COOKING))) {
				if(gc.cookedID == 884) {
					...
					if(p.getCache().hasKey("cheese_on_batta") && p.getCache().hasKey("tomato_on_batta") && !p.getCache().hasKey("onion_on_batta")) {
						...
						p.incExp(Functions.COOKING, gc.experience, true);
	...
	private boolean mouldDough(Item item, Player p) {
		if(Functions.hasItem(p, 880) || Functions.hasItem(p, 882) || Functions.hasItem(p, 883) || Functions.hasItem(p, 884) || Functions.hasItem(p, 885) || Functions.hasItem(p, 900)) {
			Functions.message(p, "you need to finish, eat or drop the unfinished dish you hold");
		...
		if(menu != -1) {
			...
			p.incExp(Functions.COOKING, 25.0, true);
			p.setBusy(false);
		}
		return true;
