...
public class DrinkMixing implements InvUseOnItemListener, InvUseOnItemExecutiveListener, InvActionListener, InvActionExecutiveListener {
	...
	public void onInvUseOnItem(Player p, Item item1, Item item2) {
		...
		if((Functions.hasItem(p, 854) || Functions.hasItem(p, 867))  && dm.itemID == 834) {
		...
		if(Functions.hasItem(p, dm.itemIDOther)) {
			p.setBusy(true);
			Functions.message(p, 1900, dm.messages[0]);
	...
	public void onInvAction(Item item, Player p) {
		if(item.getID() == SHAKER) {
			if(Functions.hasItem(p, COCKTAIL_GLASS)) {
				...
				Functions.message(p, 600, "you pour the contents into a glass");
			} else {
