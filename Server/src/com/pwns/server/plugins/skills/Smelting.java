...
InvUseOnObjectExecutiveListener { 
	...
	public void onInvUseOnObject(GameObject obj, Item item, Player p) {
		if (obj.getID() == FURNACE && !DataConversions.inArray(new int[] { GOLD_BAR, SILVER_BAR, SAND, GOLD_BAR_FAMILYCREST }, item.getID())) {
			if(item.getID() == STEEL_BAR && p.getInventory().hasItemId(CANNON_AMMO_MOULD)) {
				if (Functions.getCurrentLevel(p, Functions.SMITHING) < 30) {
				...
				Functions.message(p, 1300, "you heat the steel bar into a liquid state",
						"and pour it into your cannon ball mould",
						"you then leave it to cool for a short while");
				p.setBatchEvent(new BatchEvent(p, 2000, Formulae.getRepeatTimes(p, Functions.SMITHING)) {
					public void action() {
						p.incExp(Functions.SMITHING, 12.5, true);
						p.getInventory().replace(STEEL_BAR, MULTI_CANNON_BALL);
						Functions.addItem(p, MULTI_CANNON_BALL, 1);
	...
	private void handleRegularSmelting(final Item item, Player p, final GameObject obj) {
		if(!Functions.inArray(item.getID(), Smelt.ADAMANTITE_ORE.getID(),Smelt.COAL.getID(), Smelt.COPPER_ORE.getID(), Smelt.IRON_ORE.getID(),Smelt.GOLD.getID(), Smelt.MITHRIL_ORE.getID(),Smelt.RUNITE_ORE.getID(),Smelt.SILVER.getID(),Smelt.TIN_ORE.getID(), 690)) {
			p.message("Nothing interesting happens");
			return;
		}
		String formattedName = item.getDef().getName().toUpperCase().replaceAll(" ", "_");
		Smelt smelt;
		if(item.getID() == Smelt.IRON_ORE.getID() && Functions.getCurrentLevel(p, Functions.SMITHING) >= 30 && p.getInventory().countId(Smelt.COAL.getID()) >= 2) {
		...
		if (Functions.getCurrentLevel(p, Functions.SMITHING) < smelt.getRequiredLevel()) {
		...
		p.setBatchEvent(new BatchEvent(p, 1600, Formulae.getRepeatTimes(p, Functions.SMITHING)) {
			public void action() {
				if (p.getFatigue() >= 7500) {
					p.message("You are too tired to smelt this ore");
					interrupt();
				...
				if (p.getInventory().countId(item.getID()) > 0) {
					if(item.getID() == GOLD_BAR_FAMILYCREST - 1) 
						Functions.removeItem(p, GOLD_BAR_FAMILYCREST - 1, 1);
					else 
						Functions.removeItem(p, smelt.getID(), smelt.getOreAmount());
					...
					if (smelt.getID() == Smelt.IRON_ORE.getID() && DataConversions.random(0, 1) == 1) {
						...
						if(item.getID() == GOLD_BAR_FAMILYCREST - 1) 
							Functions.addItem(p, GOLD_BAR_FAMILYCREST, 1);
						else 
							Functions.addItem(p, smelt.getSmeltBarId(), 1);
						...
						if(p.getInventory().wielding(GAUNTLETS_OF_GOLDSMITHING) && new Item(smelt.getSmeltBarId()).getID() == GOLD_BAR) {
							p.incExp(Functions.SMITHING, smelt.getXp() + 11, true);
						} else {
							p.incExp(Functions.SMITHING, smelt.getXp(), true);
						}
					}
				} else {
					interrupt();
