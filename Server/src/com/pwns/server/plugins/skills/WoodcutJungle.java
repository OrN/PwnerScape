...
ObjectActionExecutiveListener, WallObjectActionListener, WallObjectActionExecutiveListener {
	...
	private void handleJungleWoodcut(GameObject obj, Player p) {
		...
		if(!Functions.hasItem(p, 1172)) { // machete.
			Functions.message(p, 1900, "This jungle is very thick, you'll need a machette to cut through.");
			canCut = false;
		}
		if(!Functions.hasItem(p, 1163) && !Functions.hasItem(p, 1233) && p.getQuestStage(Constants.Quests.LEGENDS_QUEST) != -1) { // the radimus scrolls.
			Functions.message(p, 1900, "This jungle is far too thick, you'll need a special map to go further.");
			canCut = false;
		}
		/* TODO 50 requirement for woodcutting? */
		if(p.getSkills().getMaxStat(Functions.WOODCUT) < 50) {
			...
			if (getLog(50, p.getSkills().getLevel(8), axeId)) {
				...
				if(jungleObject != null && jungleObject.getID() == obj.getID()) {
					if(obj.getID() == JUNGLE_VINE) {
						...
						Functions.message(p, 1200, "You hack your way through the jungle.");
					} else {
